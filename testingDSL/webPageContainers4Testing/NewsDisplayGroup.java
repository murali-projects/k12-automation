...
public class NewsDisplayGroup extends BasePageContainer{

	public NewsDisplayGroup() throws Exception {
		super();
	}
	...
	public boolean verifyNewsContentLinksArePresent() throws Exception{
		if(getResultsCount() >= 3){
			return (new TextLabel(properties.get("news_searchResults_xpath")).getXpathCount() == 3);
		}
		return new TextLabel(properties.get("news_searchResults_xpath")).getXpathCount() == getResultsCount();
	}
	...
	public boolean validateSortOrder(String searchTerm, String displayGroup, String sortBy) throws Exception{
		...
		for (int i = 1; i <= new TextLabel(properties.get("news_searchResults_xpath")).getXpathCount(); i++) {
			if(! new TextLabel(properties.get("news_searchResults_xpath")+"["+i+"]/h3/a").isPresent()){
				return false;
			}
	...
	public boolean validateDocTypes(String searchTerm, String displayGroup, String sortBy) throws Exception{
		List<String> list = new ArrayList<String>();
		for (int i = 1; i <= new TextLabel(properties.get("news_searchResults_xpath")).getXpathCount(); i++) {
			if(! new TextLabel(properties.get("news_searchResults_xpath")+"["+i+"]/").isPresent()){
				return false;
			}
			list.add(new TextLabel(properties.get("news_searchResults_xpath")+"["+i+"]/").getLabelText());
		}
	...
	public boolean verifyContentAfterClickingNewsLink() throws Exception{
		...
		return new TextLabel("").getLabelText().equals(newsTitle);
	}
