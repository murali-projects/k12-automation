...
public class ViewAllStatisticsOnSearchResultsPage extends BasePageContainer {

	private String VIEW_ALL = "View All";

	public ViewAllStatisticsOnSearchResultsPage() throws Exception {
	}
	...
	public boolean verifyNextLink() throws Exception {
		if (new TextLabel(properties.get("totalSearchResults")).isPresent())
			if (Integer
					.parseInt(getCount(properties.get("totalSearchResults"))) > 20) {
	...
	public boolean checkViewAllLinkIsWorking(String displayName) throws Exception {
		...
		return (newUrl
				.contains("StatisticsFullListPage/StatisticsFullListWindow"));
	}
	...
	public boolean validateWithDatabase(String displayGroup, String searchTerm) throws InterruptedException, Exception{
		...
		List<String> contentFromDB = OceanDatabaseReadFile.readResultsFromFile(
				properties.get("all"),properties.get(displayGroup),
		...
		for(int i=0; i<2; i++){
			...
			if(randomNumber == 0 || totalCount < randomNumber)
				continue;
			Link pageNo = new  Link("//div[@id='paging-banner']/ul/li/descendant::a[@class='current']");
			if(totalCount>20 && randomNumber>20){
				if(pageNo.isPresent() && Integer.parseInt(pageNo.getLinkText())!=((randomNumber/20)+1)){
					new Link("//div[@id='paging-banner']/ul/li/descendant::a[contains(text(),"+ ((randomNumber/20)+1) +")]").click();
				}
				validate = new TextLabel(properties.get("viewAll_searchResults")+"["+(randomNumber%20)+"]/td[2]/h3/a").getLabelText().equals(contentFromDB.get(randomNumber-1));
			}else {
				if(pageNo.isPresent() && Integer.parseInt(pageNo.getLinkText())!=((randomNumber/20)+1)){
					new Link("//div[@id='paging-banner']/ul/li/descendant::a[contains(text(),"+ ((randomNumber/20)+1) +")]").click();
				}
				validate = new TextLabel(properties.get("viewAll_searchResults")+"["+(randomNumber)+"]/td[2]/h3/a").getLabelText().equals(contentFromDB.get(randomNumber-1));
			}
			if(!validate)
				break;
		}
